...
const tiers = [
    { credits: 5, price: 0, originalPrice: 0, bestValue: false, label: "Free Trial" },
    { credits: 1000, price: 9.99, originalPrice: 24.99, bestValue: false },
    { credits: 3000, price: 19.99, originalPrice: 49.99, bestValue: false },
    { credits: 5000, price: 29.99, originalPrice: 74.99, bestValue: true },
];
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem' }}>
                    {tiers.map((tier) => (
                        <div key={tier.credits} style={{
                            ...
                            <h3 style={{ fontSize: '1.5rem', marginBottom: '0.5rem', color: 'var(--accent-ink)' }}>
                                {tier.label || `${tier.credits} Credits`}
                            </h3>
                            <p style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--accent-sepia)', marginBottom: '1.5rem' }}>
                                {tier.price === 0 ? (
                                    "Free"
                                ) : (
                                    <>
                                        <span style={{ textDecoration: 'line-through', color: '#999', fontSize: '1rem', marginRight: '0.5rem', fontWeight: 'normal' }}>${tier.originalPrice}</span>
                                        ${tier.price}
                                    </>
                                )}
                            </p>
                            <button
                                onClick={() => tier.price === 0 ? setShowPricing(false) : handleCheckout(tier.credits, tier.price)}
                                disabled={loadingTier !== null}
                                className="btn"
                                style={{ width: '100%' }}
                            >
                                {loadingTier === tier.credits ? "Processing..." : (tier.price === 0 ? "Start Free" : "Select Plan")}
                            </button>
                        </div>
                    ))}
                </div>
